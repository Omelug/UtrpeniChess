...
class Pawn(Figure):
    def move(self, to_x, to_y, target=None, realize=True):
        ...
        _, adb_d_y = abs_delta(to_x, to_y, self.figure)
        if first_step and (not free_place(to_x, to_y - adb_d_y / 2, self.map_jso) or not free_place(to_x, to_y, self.map_jso)):
            logging.error(f"Invalid move: first step ({to_x};{ to_y - adb_d_y / 2}) {first_step} {free_place(to_x, to_y - delta_y / 2, self.map_jso)} {free_place(to_x, to_y, self.map_jso)}")
            return False
